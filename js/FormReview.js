document.addEventListener('DOMContentLoaded', function() {
    ...
    // Kullanıcı konumunu al ve formları filtrele
    const forms = JSON.parse(localStorage.getItem('forms')) || [];
    console.log('Forms:', forms);

    const filteredForms = forms.filter(form => form.position === loggedInUserPosition);
    ...
    if (filteredForms.length > 0) {
        ...
        formsContainer.innerHTML = '<p>Üzerinize atanmış form bulunmamaktadır.</p>';
    }
});
